export {
  ApiClient,
  ApiError,
  type ApiClientOptions,
  type Schemas,
  type UserMe,
  type LoginRequest,
  type LoginResponse,
  type AuthMethodsOut,
  type TOTPCodeIn,
  type TOTPEnrollOut,
  type TOTPStatusOut,
  type PasskeyOut,
  type PasskeyStatusOut,
  type PasskeyRegisterFinishIn,
  type PasskeyAuthenticateFinishIn,
  type PasskeyOptionsJSON,
  type QueueNode,
  type QueueCounts,
  type TicketListItem,
  type PaginatedTickets,
  type MyTicketCounts,
  type DashboardSummary,
  type TicketDetail,
  type TicketPermissions,
  type ArticleListItem,
  type ArticleBody,
  type AttachmentMetaOut,
  type HistoryEntry,
  type CustomerUserOut,
  type SearchHit,
  type SearchResponse,
  type SimilarTicketItem,
  type SimilarTicketsOut,
  type CustomerCompanyRefOut,
  type CustomerContactRefOut,
  type CustomerSearchOut,
  type DynamicFieldValueOut,
  type PresenceIn,
  type PresenceEntry,
  type OnlineAgentOut,
  type ArticleCreateRequest,
  type ArticleCreateResponse,
  type ReplyDraftOut,
  type AiOriginOut,
  type TemplateOut,
  type MutationRequest,
  type MergeRequest,
  type AcquireLockRequest,
  type AcquireLockResponse,
  type TicketCreateRequest,
  type TicketCreateResponse,
  type ForwardRequest,
  type BounceRequest,
  type SplitRequest,
  type TicketLinkTargetOut,
  type TicketLinkCreateRequest,
  // Portal
  type CustomerMe,
  type CustomerLoginResponse,
  type PortalArticleOut,
  type PortalReplyRequest,
  type PortalReplyResponse,
  type PortalTicketCreateRequest,
  type PortalTicketCreateResponse,
  type PortalAttachmentUploadResponse,
  type KbSearchHit,
  type KbSearchResponse,
  // Knowledge base (agent)
  type CategoryOut,
  type CategoryIn,
  type CategoryUpdateIn,
  type KbArticleOut,
  type KbArticleIn,
  type KbArticleUpdateIn,
  type ArticleSummary,
  type ArticleVersionOut,
  type AssignableGroup,
  type KbTagOut,
  type KbAttachmentOut,
  type KnowledgeArticle,
  type KnowledgeBundle,
  // Admin
  type AdminPage,
  type AdminListParams,
  type AdminValidFilter,
  type UserOut,
  type UserCreate,
  type UserUpdate,
  type GroupOut,
  type GroupCreate,
  type GroupUpdate,
  type GroupAssignment,
  type RoleOut,
  type RoleCreate,
  type RoleUpdate,
  type RoleAssignment,
  type GroupRoleAssignment,
  type EffectivePermissionsOut,
  type EffectiveGroupPermission,
  type EffectiveQueuePermission,
  type EffectivePermissionSource,
  type UserLanguageOut,
  type UserDeletableOut,
  type PasswordSetupCheckOut,
  type UserReference,
  type UserLanguageUpdate,
  type QueueOut,
  type QueueCreate,
  type QueueUpdate,
  type StateOut,
  type StateCreate,
  type StateUpdate,
  type PriorityOut,
  type PriorityCreate,
  type PriorityUpdate,
  type TicketTypeOut,
  type TicketTypeCreate,
  type TicketTypeUpdate,
  type ServiceOut,
  type ServiceCreate,
  type ServiceUpdate,
  type SlaOut,
  type SlaCreate,
  type SlaUpdate,
  type SystemAddressCreate,
  type SystemAddressUpdate,
  type NotificationEventOut,
  type NotificationEventWrite,
  type NotificationEventUpdate,
  type NotificationMessageIn,
  type GenericAgentJobWrite,
  type GenericAgentJobUpdate,
  type MentionOut,
  type MentionCreate,
  type TimeAccountingOut,
  type TimeAccountingCreate,
  type TimeAccountingReportEntry,
  type TimeAccountingReportOut,
  type TypeRef,
  type ServiceRef,
  type SlaRef,
  type CustomerUserAdminOut,
  type CustomerUserAdminCreate,
  type CustomerUserAdminUpdate,
  type CustomerUserBulkUpdate,
  type CustomerUserBulkUpdateResult,
  type CustomerCompanyOut,
  type CustomerCompanyCreate,
  type CustomerCompanyUpdate,
  type SalutationOut,
  type SalutationWrite,
  type SalutationUpdate,
  type SignatureOut,
  type SignatureWrite,
  type SignatureUpdate,
  type StandardTemplateOut,
  type StandardTemplateCreate,
  type StandardTemplateUpdate,
  type StandardAttachmentOut,
  type StandardAttachmentCreate,
  type StandardAttachmentUpdate,
  type AttachmentRefOut,
  type TemplateAttachmentsReplace,
  type CustomerUserGroupAssignment,
  type AutoResponseOut,
  type AutoResponseCreate,
  type AutoResponseUpdate,
  type DynamicFieldOut,
  type DynamicFieldCreate,
  type DynamicFieldUpdate,
  type WebhookOut,
  type WebhookCreate,
  type WebhookUpdate,
  type AuthConfigAgentOut,
  type AuthConfigUpdate,
  type AuthConfigGlobalOut,
  type AuthConfigGlobalUpdate,
  type ApiKeyOut,
  type ApiKeyCreate,
  type ApiKeyUpdate,
  type ApiKeyCreated,
  type QueueVariableOut,
  type QueueVariableCreate,
  type QueueVariableUpdate,
  type PhysicalQueueVariableOut,
  type QueueCustomerLinkOut,
  type QueueCustomerLinkCreate,
  type QueueCustomerLinkUpdate,
  type ResolvedCustomerLink,
  type PlaceholderFieldOut,
  type PlaceholderFieldCreate,
  type PlaceholderFieldUpdate,
  type DaemonServiceOut,
  type DaemonListOut,
  type DaemonUpdate,
  type SystemInfoOut,
  type MailSecurity,
  type MailAuthType,
  type MailOutboundOut,
  type MailOutboundUpdate,
  type MailOutboundTestIn,
  type MailOutboundTestOut,
  type OAuth2TokenConfigOut,
  type OAuth2TokenConfigCreate,
  type OAuth2TokenConfigUpdate,
  type OAuth2AuthorizeUrlOut,
  type OAuth2ProviderTemplateOut,
  type MailAccountType,
  type MailAccountAuthType,
  type MailAccountOut,
  type MailAccountCreate,
  type MailAccountUpdate,
  type ChannelConfigOut,
  type ChannelConfigUpdate,
  type TelegramWebhookRegisterRequest,
  type TelegramWebhookRegisterResponse,
  type TelegramWebhookResponse,
  type SubjectFormat,
  type SubjectHookZnunyOut,
  type SubjectHookOverridesOut,
  type SubjectConfigOut,
  type SubjectConfigUpdate,
  type MailLogDirection,
  type MailLogStatus,
  type MailLogOut,
  type MailLogListParams,
  type ErasureMode,
  type ErasureSelectorIn,
  type GdprErasurePreviewRequest,
  type GdprResolvedCustomerOut,
  type GdprSampleRowOut,
  type GdprErasurePreviewOut,
  type GdprErasureJobCreate,
  type GdprErasureJobOut,
  type GdprErasureJobDetailOut,
  type GdprJobListParams,
  type GdprRollbackOut,
  type GdprPurgeOut,
  type GdprSelectorCountRequest,
  type GdprSelectorCountOut,
  type GdprCustomerRecordPreviewRequest,
  type GdprFieldPreviewOut,
  type GdprDeleteSummaryRowOut,
  type GdprCustomerRecordPreviewOut,
  type PostmasterFilterOut,
  type PostmasterFilterRuleOut,
  type PostmasterFilterWrite,
  type PostmasterMatchRuleIn,
  type PostmasterSetRuleIn,
  type AclOut,
  type AclCreate,
  type AclUpdate,
  type GenericAgentJobOut,
  type SystemAddressOut,
  type FollowUpPossibleOut,
  // Stats
  type TicketVolumeOut,
  type VolumePointOut,
  type OpenSnapshotOut,
  type DimensionCountOut,
  type SlaStatsOut,
  type AgentWorkloadItemOut,
  type BacklogTrendOut,
  type BacklogPointOut,
  type StatsGranularity,
  type StatsDimension,
  type StatsFilterParams,
  // Calendar
  type CalendarOut,
  type RecurrenceIn,
  type AppointmentIn,
  type AppointmentUpdateIn,
  type AppointmentOut,
  type OccurrenceOut,
  type TicketLinkOut,
  // ProcessManagement (BPM)
  type ProcessSummaryOut,
  type ActivityDialogSummaryOut,
  type ActivityDialogRefOut,
  type ProcessActivityOut,
  type ProcessDetailOut,
  type TicketProcessStateOut,
  type ActivityDialogFieldOut,
  type ActivityDialogDetailOut,
  type ProcessStartIn,
  type ActivityDialogSubmitIn,
  type ActivityDialogSubmitOut,
  // Reference (agent pickers)
  type PriorityRef,
  type StateRef,
  type AgentRef,
  type CustomerRef,
  type QueueRef,
  type ComposeContext,
  type TicketSearchHit,
  type AgentCustomerCreateInput,
  type AgentCustomerCreateOut,
  type AgentTicketCreateInput,
  type paths,
} from "./client";
