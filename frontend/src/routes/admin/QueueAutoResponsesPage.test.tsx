...
const listQueueAutoResponses = vi.fn();
const listAutoResponseQueues = vi.fn();
...
vi.mock("@/lib/api", () => ({
  api: {
    ...
    listQueueAutoResponses: (...args: unknown[]) => listQueueAutoResponses(...args),
    listAutoResponseQueues: (...args: unknown[]) => listAutoResponseQueues(...args),
...
describe("QueueAutoResponsesPage", () => {
  beforeEach(() => {
    ...
    listQueueAutoResponses.mockReset();
    listAutoResponseQueues.mockReset();
    ...
    listQueueAutoResponses.mockResolvedValue([]);
    listAutoResponseQueues.mockResolvedValue([]);
    assignQueueAutoResponse.mockResolvedValue(undefined);
    revokeQueueAutoResponse.mockResolvedValue(undefined);
  });

  it("submits PUT assign when an auto-response is toggled on", async () => {
    renderPage();

    await screen.findByTestId("admin-queue-auto-responses-page-anchor-7");
    fireEvent.click(screen.getByTestId("admin-queue-auto-responses-page-anchor-7"));

    await screen.findByTestId("admin-queue-auto-responses-page-counterpart-11");
    fireEvent.click(screen.getByTestId("admin-queue-auto-responses-page-counterpart-11"));
