...
/**
 * Rich, representative mock dataset for README screenshots and the MSW demo.
 * Self-contained (does NOT touch the e2e fixtures used by the test suite) so it
 * can be as full as we like. English data. Admin resource lists return the
 * `AdminPage` envelope ({items,total,page,page_size}); agent endpoints match the
 * real API shapes.
 */
...
// Portal
const portalUser = { login: CUSTOMERS[0].login, customer_id: "ACME", first_name: "Jane", last_name: "Doe", email: CUSTOMERS[0].login };
const portalTickets = { items: ticketItems.slice(0, 4).map((t) => ({ id: t.id, tn: t.tn, title: t.title, state: t.state, state_type: t.state_type, queue_name: t.queue_name, create_time: t.create_time, change_time: t.change_time })), total: 4, offset: 0, limit: 50 };

function route(path: string, method: string): unknown | undefined {
  const p = path;
  // Auth
  ...
  if (p.endsWith("/auth/login") && method === "POST") return { user };
...
export async function mockRich(page: Page) {
  ...
  await page.route("**/api/**", async (r) => {
    ...
    if (p.endsWith("/events/stream")) { await r.fulfill({ status: 200, contentType: "text/event-stream", body: "" }); return; }
    // Agent auth plane (flag so the login form → /agent redirect works).
    if (p.endsWith("/api/v1/auth/methods")) { await json(r, 200, { password: true, oidc: false, spnego: false, webauthn: true, ldap: false }); return; }
    if (p.endsWith("/api/v1/auth/login") && method === "POST") { authed = true; await json(r, 200, { user }); return; }
    if (p.endsWith("/api/v1/auth/me")) { if (!authed) { await json(r, 401, { detail: "Not authenticated" }); return; } await json(r, 200, user); return; }
