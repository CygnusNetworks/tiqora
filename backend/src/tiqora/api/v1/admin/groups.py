...
async def list_groups(admin: AdminUser, session: DbSession) -> list[PermissionGroups]:
    _ = admin
    result = await session.execute(select(PermissionGroups).order_by(PermissionGroups.name))
